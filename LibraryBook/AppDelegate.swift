...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        let task = URLSession.shared.dataTask(with: request) { (data, response, oError) -> Void in
            
            var libraryBooks: [Library.Book] = []
            
            if let data = data {
                do {
                    if let jsonDictionaries = try JSONSerialization.jsonObject(with: data, options: []) as? [[String:Any]] {
                ...
                print("failed with response \(_response)")
            } else {
                print("system error: \(oError!)")
            }
        }
        task.resume()
